...
export function getMouseClickDelay() {
...
/** Mouse move event */
function bezierCurve(x0, y0, x1, y1, x2, y2, x3, y3, numPoints) {
  const points = [];
...
export function generateHumanMousePath(
  ...
  const calculatedPath = path.map((point, index) => {
...
export async function getMouseImage() {
  if (!mouseImage) {
